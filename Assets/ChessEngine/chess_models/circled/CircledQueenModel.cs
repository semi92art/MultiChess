...
{
    public sealed class CircledQueenModel : ChessItemModelBase
    {
        public CircledQueenModel(ChessSide side, BoardPosition pos, int steps = 0) : base(side, (byte)CircledChessItemType.queen, pos, steps) { }
        public override List<BoardPosition> GetPossibleMoves(IChessItemModel[,] items, out List<bool> isJumpList, out List<bool> killPossibilityList)
        {
            ...
            foreach (var poss_move in PossibleMovesFinder.GetRookPossibleMoves(items, Side, Pos, Steps, false, true, out isJumpList1, out killPossibilityList1))
            {
                result.Add(poss_move);
            }

            foreach (var poss_move in PossibleMovesFinder.GetBishopPossibleMoves(items, Side, Pos, true, out isJumpList2, out killPossibilityList2))
            {
                if (poss_move != Pos)
                    result.Add(poss_move);
            }
